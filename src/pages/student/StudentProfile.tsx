...
export function StudentProfile() {
  ...
  // The profile is already resolved by the app-level loading gate before this
  // page renders, so we can seed the input directly.
  ...
  const handleSave = async () => {
    ...
    setSaved(false);
    try {
      await updateDoc(doc(db, 'users', currentUser.uid), { whatsappPhone: cleaned });
      setSaved(true);
    } catch {
    ...
    <Layout title="Meu Perfil" backTo="/student">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Atualize seu número de WhatsApp usado nas notificações de treino.
        </p>
      </div>
      ...
      <div className="glass-premium flex flex-col gap-5 rounded-2xl p-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500">
            O e-mail está vinculado à sua conta Google e não pode ser alterado.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500">
            Inclua o código do país. Somente você e seus treinadores verão este número.
          </p>
        </div>
