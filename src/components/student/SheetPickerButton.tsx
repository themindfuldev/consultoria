...
export function SheetPickerButton({ sheetName, picking, onClick }: SheetPickerButtonProps) {
  return (
    ...
    >
      ...
      <span className={`flex-1 truncate ${sheetName ? '' : 'text-slate-400 dark:text-slate-500'}`}>
        {picking ? 'Abrindo o Google…' : sheetName || 'Selecionar planilha do Google Sheets'}
      </span>
      <span className="flex-shrink-0 text-xs font-semibold text-indigo-600 dark:text-indigo-400">
        {sheetName ? 'Trocar' : 'Selecionar'}
      </span>
    </button>
  );
}
